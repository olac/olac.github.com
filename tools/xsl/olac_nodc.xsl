...
OAI_DC support removed for experimental purposes:  GFS, 2002-08-23
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:java="http://xml.apache.org/xslt/java" exclude-result-prefixes="java" version="1.0">
	...
	<xsl:variable name="recordId" select="substring-after($identifier, $idPrefix)"/>
...
-->
	<xsl:template name="Identify">
		<Identify xmlns="http://www.openarchives.org/OAI/1.1/OAI_Identify" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.openarchives.org/OAI/1.1/OAI_Identify         
			...
			<description>
				<oai-identifier xmlns="http://www.openarchives.org/OAI/1.1/oai-identifier" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.openarchives.org/OAI/1.1/oai-identifier   
...
-->
	<xsl:template name="ListSets">
		...
		<ListSets xmlns="http://www.openarchives.org/OAI/1.1/OAI_ListSets" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.openarchives.org/OAI/1.1/OAI_ListSets     
...
-->
		<ListIdentifiers xmlns="http://www.openarchives.org/OAI/1.1/OAI_ListIdentifiers" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.openarchives.org/OAI/1.1/OAI_ListIdentifiers         
					...
					<xsl:choose>
						<xsl:when test="($from!='') and ($until!='')">
							<xsl:if test="(not(java:before($ds, $datefrom))) and (not(java:after($ds, $dateuntil)))">
								<identifier>
									<xsl:value-of select="concat($idPrefix,header/recordId)"/>
								</identifier>
							</xsl:if>
						</xsl:when>
						<xsl:when test="($from!='') and ($until='')">
						...
						<xsl:otherwise>
							<identifier>
								<xsl:value-of select="concat($idPrefix,header/recordId)"/>
								<!-- Can't imagine why this is here, GFS							 <xsl:value-of select="$from"/> -->
							</identifier>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:if>
...
-->
	<xsl:template name="ListMetadataFormats">
		<!-- 	identifier		#OPTIONAL -->
		<ListMetadataFormats xmlns="http://www.openarchives.org/OAI/1.1/OAI_ListMetadataFormats" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.openarchives.org/OAI/1.1/OAI_ListMetadataFormats         
...
-->
	<xsl:template name="GetRecord">
		...
		<GetRecord xmlns="http://www.openarchives.org/OAI/1.1/OAI_GetRecord" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.openarchives.org/OAI/1.1/OAI_GetRecord         
...
-->
	<xsl:template name="ListRecords">
		<!--	metadataPrefix	#REQUIRED	
				...
				set			#OPTIONAL
				resumptionToken	#EXCLUSIVE	(not implemented)	-->
		<ListRecords xmlns="http://www.openarchives.org/OAI/1.1/OAI_ListRecords" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.openarchives.org/OAI/1.1/OAI_ListRecords  http://www.openarchives.org/OAI/1.1/OAI_ListRecords.xsd">
			<xsl:call-template name="dateAndURL"/>
			<xsl:for-each select="/OLAC-Repository/records/*">
				<xsl:if test="($set='') or (@spec=$set)">
					<xsl:variable name="datestamp" select="header/datestamp"/>
